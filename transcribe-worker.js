...
// 같은 출처(self) 모듈 워커라서 CDN import가 안정적으로 동작한다.
...
let key = '';      // 현재 로드된 (모델|디바이스|양자화) 조합

self.onmessage = async (e) => {
  const m = e.data;
  if(m.type !== 'run') return;
  try{
    const k = m.modelId + '|' + m.device + '|' + m.quant;
    if(!tr || key !== k){
      tr = await pipeline('automatic-speech-recognition', m.modelId, {
        device: m.device,                                   // 'webgpu' 또는 'wasm'
        dtype: m.device === 'webgpu' ? 'fp16' : (m.quant ? 'q8' : 'fp32'),
