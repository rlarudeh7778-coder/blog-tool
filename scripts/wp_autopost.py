...
# ───────────────────────── 프롬프트 ─────────────────────────
COMMON_RULES = (
    "절대 준수(★구글 애드센스 승인 최우선★):\n"
    "1. 제목은 키워드 단독 나열 금지 — 검색자의 고민을 그대로 담은 후킹 문구로(32자 이내).\n"
    "2. 공백포함 3,200~4,200자로 충분히 깊게. 단 분량 채우기용 군더더기·동어반복 절대 금지 — "
    "모든 문장이 새 정보를 줄 것. 빈약하면 발행 가치가 없다.\n"
    "3. ## ** 등 마크다운 기호 금지(순수 텍스트 줄바꿈만).\n"
    "4. ★결론 먼저★ 도입 첫 1~2문장에 핵심 답/결론을 먼저. 이어서 '왜 그런지'를 궁금하게 만드는 후킹 1문장.\n"
    "5. 소제목 6개 이상 — 검색자가 진짜 궁금해할 세부 질문 단위로 쪼갤 것. 그중 2~3개는 '~까요?' 질문형(FAQ 자동화).\n"
    "6. 한 문단은 1~3문장, 모든 문장은 마침표/물음표로 종결(모바일 가독성).\n"
    "7. ★깊이★ 단순 나열이 아니라 '왜 그런지'를 설명하고, 실제 화면/메뉴 경로·버튼명·단축키·예상 소요시간·수치를 "
    "구체적으로. 컴퓨터를 잘 모르는 초보가 그대로 따라 해도 끝까지 해결되도록 단계마다 빠짐없이.\n"
    "8. ★독창성(E-E-A-T)★ 어디에나 있는 뻔한 원론 말고, 실제 경험에서 나오는 팁·자주 하는 실수·다른 글엔 잘 없는 "
    "디테일을 2개 이상 넣을 것. 베낀 듯한 일반론과 AI 상투어('~를 통해', '~할 수 있습니다' 남발) 금지.\n"
    "9. 강조 꿀팁은 한 줄 '[팁] ...', 주의는 '[주의] ...' 형식(각 1~2개).\n"
    "10. 구체적 수치·버전·모델명 등 사실 정보 4개 이상. 단 확실하지 않은 사실을 지어내지 말 것.\n"
    "11. summary는 글 핵심(결론)을 25자 이내 3개로 — 맨 위 요약으로 노출됨.\n"
    "12. ★애드센스 정책★ '독자의 문제를 이 글 하나로 완전히 끝내주는 완결성 있는 정보글'이 목표. "
    "키워드만 채운 얇은 글, 어디에나 있는 뻔한 글, 광고 유도가 주가 되는 글은 절대 금지(=저품질 콘텐츠로 거절됨).\n"
...
def gen_it(topic, reference, model):
    ...
    user = (
        "아래 주제로, 검색한 사람이 이 글 하나로 문제를 완전히 끝내는 깊이 있는 IT 정보 글을 써라.\n"
        f"주제: {topic}\n"
        "구성: (1) 어떤 상황인지 1~2문장 공감 도입(+결론 먼저) (2) 원인 2~3가지를 '왜 그런지'까지 설명 "
        "(3) 해결방법을 가장 쉽고 가능성 높은 것부터 단계별 번호 소제목으로 — 메뉴 경로·버튼명·단축키·예상 소요시간까지 구체적으로 "
        "(4) 그래도 안 될 때의 추가 점검·대안 (5) 예방법이나 초보가 자주 하는 실수 1~2개로 마무리.\n"
        "기종·윈도우 버전 등 상황별로 방법이 갈리면 어디서 갈리는지 짚어줄 것.\n"
        + ref + "\n" + COMMON_RULES
    )
    return parse_json(claude(system, user, model, max_tokens=6000))
...
def main():
    ...
    # ★주제 중복 완전 차단: 참고자료 제목과 겹치는 IT주제 제거 + 각 목록 내부 중복 제거(순서 유지)
    _ref_titles = {(r.get("title") or "").strip() for r in references}
    def _dedup(seq):
        seen, out = set(), []
        for x in seq:
            x = (x or "").strip()
    ...
    for _ in range(posts):
        state["counter"] += 1
        # 선불폰 차례인지(phone_every 편마다) + 아직 남은 선불폰 키워드가 있는지
        want_phone = (phone_every > 0 and phone_kws and state["counter"] % phone_every == 0)
        phone_left = state["phone_idx"] < len(phone_kws)
        is_phone = want_phone and phone_left
        try:
            if is_phone:
                # ★중복 금지: 키워드를 1회씩만 사용(순환 없음)
                kw = phone_kws[state["phone_idx"]]
                state["phone_idx"] += 1
                ...
                kind = "it"
                # 1) 참고자료(있으면 우선, 1자료=1글) → 2) IT 주제 뱅크 → 모두 1회씩만(순환 없음)
                if references and state["ref_idx"] < len(references):
                    ...
                    label = f"[IT/각색] {topic}"
                elif state["it_idx"] < len(it_topics):
                    topic = it_topics[state["it_idx"]]
                    state["it_idx"] += 1
                    post = gen_it(topic, "", model)
                    label = f"[IT] {topic}"
                elif phone_left:
                    # IT가 다 떨어졌으면 남은 선불폰으로
                    kw = phone_kws[state["phone_idx"]]
                    state["phone_idx"] += 1
                    post = gen_phone(kw, contact.get("brand", "메이플통신"),
                                     cfg.get("platform_url", "https://www.n-telecom.co.kr/junda"), model)
                    ...
                    print("✅ 모든 주제를 1회씩 발행 완료(중복 방지). config에 주제를 추가하면 이어서 발행됩니다.")
                    break
